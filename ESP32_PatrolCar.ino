...
void Forward(){move(1,50,1);move(2,50,1);}//前进
void Backward(){move(1,50,2);move(2,50,2);}//后退
void Left(){move(1,50,2);move(2,50,1);}//左移
void Round(){move(1,50,1);move(2,50,2);}//右移
...
void forward(int i)
{
  ...
  { 
    ...
    if( (D5==HIGH || D6==HIGH) || (D7==HIGH || D8==HIGH) )
    {
      y++;
    }
  }
...
void left(int i)
{
  ...
  while(x<=i)
  {
    read();
    Left();
    if( (D1==HIGH || D2==HIGH) || (D3==HIGH || D4==HIGH) )
    {
      x++;
    }
  }
...
{ 
  ...
  if(Button2==HIGH)
  {
    ...
    {
      FunctionTwo();//"4"
      Serial.println("functiontwo_4");
    }

    if(targetX==8)
    {
      FunctionThree();//"3"
      Serial.println("functionthree_3");   }
    if(targetX==9)
    {
      FunctionFour();//"王"
      Serial.println("functionfour_wang");
    }
    else
    {
      FunctionFive();//五点循迹
      Serial.println("functionfive_Fivepoint");
    }
  }
}
